    ...
    public class LoadXMLStart : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            foreach (GameObject character in GameObject.FindGameObjectsWithTag("Mayor Male"))
            {
                ...
                CD.WorkEthicScore = Convert.ToInt32(CDXML.GetElementsByTagName("WorkEthicScore").Item(0).InnerText);
                ...
                CharacterWorldData CWD = character.AddComponent<CharacterWorldData>();
                
                // Info
                Vector3Int tilemapPos = WorldToolManager.current.tilemapList[0].WorldToCell(character.transform.position);
                CWD.WorldPositionX = tilemapPos.x;
                CWD.WorldPositionY = tilemapPos.y;
                CWD.WorldPositionZ = tilemapPos.z;
                ...
                CSD = CSD.ToSaveData(CD);
                XMLGameManager.XMLGameData.Characters.Add(CSD);

                CharacterWorldSaveData CWSD = new CharacterWorldSaveData();
                CWSD = CWSD.ToSaveData(CWD);
                XMLGameManager.XMLGameData.CharactersWorldData.Add(CWSD);

                // Component: MouseoverStats

                character.AddComponent<MouseoverStatsTrigger>();
